# Multipart2 [![Build Status](https://travis-ci.org/nbittich/multipart2.svg?branch=master)](https://travis-ci.org/nbittich/multipart2) [![On Crates.io](https://img.shields.io/crates/v/multipart2.svg)](https://crates.io/crates/multipart2)
...
### [Documentation](http://docs.rs/multipart2/)

## Integrations

Example files demonstrating how to use `multipart2` with these crates are available under [`examples/`](examples).
...
Client integration includes support for regular `hyper::client::Request` objects via `multipart2::client::Multipart`, as well
as integration with the new `hyper::Client` API via `multipart2::client::lazy::Multipart` (new in 0.5).

Server integration for `hyper::server::Request` via `multipart2::server::Multipart`.

### [Iron ![](https://img.shields.io/crates/v/iron.svg)](https://crates.io/crates/iron) 
via the `iron` feature.

Provides regular server-side integration with `iron::Request` via `multipart2::server::Multipart`, 
as well as a convenient `BeforeMiddleware` implementation in `multipart2::server::iron::Intercept`.

### [Nickel ![](https://img.shields.io/crates/v/nickel.svg)](https://crates.io/crates/nickel) <sup>returning to `multipart2` in 0.14!</sup>
via the `nickel` feature.

Provides server-side integration with `&mut nickel::Request` via `multipart2::server::Multipart`. 

### [tiny_http ![](https://img.shields.io/crates/v/tiny_http.svg)](https://crates.io/crates/tiny_http)
via the `tiny_http` feature.

Provides server-side integration with `tiny_http::Request` via `multipart2::server::Multipart`.
...
behind the scenes which would supercede any integration with `multipart2`. However, an example is available
showing how to use `multipart2` on a Rocket server: [examples/rocket.rs](examples/rocket.rs)
